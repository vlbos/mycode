...
若申请通过，学生就可以在第 $i$ 个时间段去教室 $d\_i$ 上课，否则仍然在教室 $c\_i$ 上课。
...
当第 $i$ ($1 \\le i \\le n-1$) 节课结束后，牛牛就会从这节课的教室出发，选择一条耗费体力最少的路径前往下一节课的教室。
...
第四行 $n$ 个实数，第 $i$ 个实数表示 $k\_i$，即牛牛申请在第 $i$ 个时间段更换教室获得通过的概率；保证 $0 \\le k\_i \\le 1$。
