...
游戏开始时，给定玩家一个具有N个顶点N条边（编号 1∼N1∼N）的多边形，如图1所示，其中 N\=4N\=4。
...
接下来在进行 N−1N−1 步，在每一步中，玩家选择一条边，把这条边以及该边连接的两个顶点用一个新的顶点代替，新顶点上的整数值等于删去的两个顶点上的数按照删去的边上标有的符号进行计算得到的结果。
...
例题/0x50 动态规划/0x53 区间DP/Polygon
