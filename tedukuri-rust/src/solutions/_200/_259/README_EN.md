...
每组测试用例的第一行包含三个非负整数 $n,p\_1,p\_2$，其中 $n$ 是你可以提问的总次数，$p\_1$ 是天神的总数量，$p\_2$ 是恶魔的总数量。

接下来 $n$ 行每行包含两个整数 $x\_i,y\_i$ 以及一个字符串 $a\_i$，其中 $x\_i,y\_i$ 是岛上居民的编号，你将向编号为 $x\_i$ 的居民询问编号为 $y\_i$ 的居民是否是天神，
