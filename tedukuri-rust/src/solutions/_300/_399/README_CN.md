...
已知约翰所在的城镇，有n条街道（编号1到n），mm 个路口（编号1到m），每条街道都被两个路口连接，任意两条街道之间都是互通的。
...
每组测试用例包含若干行，每行包含三个整数 x，y，zx，y，z，表示路口x和路口y之间有一条街道z。

**假设每组测试用例中，第一行输入的路口x和 路口y中，数值较小的那个路口，即为约翰的家所在的路口。**
