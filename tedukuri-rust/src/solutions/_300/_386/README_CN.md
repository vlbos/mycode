...
我们将这个关系网络对应到一个n个结点的无向图上，两个不同的人若互相认识，则在他们对应的结点之间连接一条无向边，并附上一个正数权值c，cc 越小，表示两个人之间的关系越密切。
...
我们修改重要程度的定义如下：令 Cs,tCs,t 表示从s到t的不同的最短路的数目，Cs,t(v)Cs,t(v) 表示经过v的从s到t的最短路的数目；

则定义

I(v)\=∑s≠v,t≠vCs,t(v)Cs,tI(v)\=∑s≠v,t≠vCs,t(v)Cs,t

为结点v在社交网络中的重要程度。

为了使 I(v)I(v) 和 Cs,t(v)Cs,t(v) 有意义，我们规定需要处理的社交网络都是连通的无向图，即任意两个结点之间都有一条有限长度的最短路径。
...
接下来m行，每行用三个整数 a，b，ca，b，c 描述一条连接结点a和b，权值为c的无向边。
...
n≤100,m≤4500,1≤c≤1000n≤100,m≤4500,1≤c≤1000
