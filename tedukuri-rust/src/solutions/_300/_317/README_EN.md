...
设密文中每行前 $4$ 个数依次为 $L\_1，L\_2，L\_3，D$，求出所有深度为 $D$，含有 $L\_1$ 对 `{}`，$L\_2$ 对 `[]`，$L\_3$ 对 `()` 的 $SS$ 串的个数，并用这个数对当前的年份 $11380$ 求余数，这个余数就是密文中每行的第 $5$ 个数，我们称之为神秘数。
