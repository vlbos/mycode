...
通过 $2$ 个栈 $S\_1$ 和 $S\_2$，Tom 希望借助以下 $4$ 种操作实现将输入序列升序排序。

操作 $a$

如果输入序列不为空，将第一个元素压入栈 $S\_1$

操作 $b$

如果栈 $S\_1$ 不为空，将 $S\_1$ 栈顶元素弹出至输出序列

操作 $c$

如果输入序列不为空，将第一个元素压入栈 $S\_2$

操作 $d$

如果栈 $S\_2$ 不为空，将 $S\_2$ 栈顶元素弹出至输出序列
