...
impl Excel {
    pub fn new(h: i32, w: char) -> Self {
        ...
        //     cols,
        // }
        Self{mat:vec![vec![0;h as usize];(w as u8-b'A') as usize+1],m:HashMap::new()}
    }
    ...
    pub fn sum(&mut self, r: i32, c: char, strs: Vec<String>) -> i32 {
        ...
        for str in &strs{
            if let Some(k)=str.rfind(":"){
                let (x1,y1)=(str[1..k].parse::<usize>().unwrap(),(str.as_bytes()[0]-b'A') as usize);
                let (x2,y2)=(str[k+2..].parse::<usize>().unwrap(),(str.as_bytes()[k+1]-b'A') as usize);
                for i in x1..=x2{
                    for j in y1..=y2{
                ...
                ans+=self.get(str[1..].parse::<i32>().unwrap(),str.chars().nth(0).unwrap());
            }
        }
...
mod test {
    ...
    fn test_excel_1() {
        ...
        assert_eq!(
            excel.mat,
            lc_matrix![[0, 0, 0], [0, 0, 0], [0, 0, 0]]
        );
        excel.set(1, 'A', 2);
        assert_eq!(
            excel.mat,
            lc_matrix![[2, 0, 0], [0, 0, 0], [0, 0, 0]]
        );
