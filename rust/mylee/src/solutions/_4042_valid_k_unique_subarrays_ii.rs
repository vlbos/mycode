...
//  vector<bool> valid_subarrays(vector<int>& nums, int k, int l0, int r0, int q) {


impl Solution {
    pub fn valid_subarrays(nums: Vec<i32>, k: i32, l0: i32, r0: i32,q:i32) -> Vec<bool> {
        vec![]
...
mod test {
    ...
    pub fn test_valid_subarrays_1() {
        assert_eq!(
            Solution::valid_subarrays(vec![1,2,2,1],2,1, 2, 2),
            vec![false,true]
        );
    }
    ...
    pub fn test_valid_subarrays_2() {
        assert_eq!(
            Solution::valid_subarrays(vec![1,2,3,3,4], 1, 2,3, 2),
            vec![true,false]
        );
    }
