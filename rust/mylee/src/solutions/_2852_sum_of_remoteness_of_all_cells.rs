...
// The top-left grid contains the initial values in the grid.
// Blocked cells are colored black, and other cells get their values as it is in the input.
// In the top-right grid, you can see the value of R[i][j] for all cells.
// So the answer would be the sum of them. That is: 0 + 12 + 0 + 8 + 0 + 9 + 0 + 10 + 0 = 39.
// Let 's jump on the bottom-left grid in the above picture and calculate R[0][1] (the target cell is colored green).
//  We should sum up the value of cells that can 't be reached by the cell (0, 1).
// These cells are colored yellow in this grid. So R[0][1] = 5 + 4 + 3 = 12.
// Now let 's jump on the bottom-right grid in the above picture and calculate R[1][2] (the target cell is colored green).
// We should sum up the value of cells that can 't be reached by the cell (1, 2).
// These cells are colored yellow in this grid. So R[1][2] = 1 + 5 + 3 = 9.
//
...
// Blocked cells are colored black, and other cells get their values as it is in the input.
// In the top-right grid, you can see the value of R[i][j] for all cells.
// So the answer would be the sum of them. That is: 3 + 3 + 0 + 0 + 0 + 0 + 7 + 0 + 0 = 13.
// Let 's jump on the bottom-left grid in the above picture and calculate R[0][2] (the target cell is colored green).
// We should sum up the value of cells that can 't be reached by the cell (0, 2).
// This cell is colored yellow in this grid. So R[0][2] = 3.
// Now let 's jump on the bottom-right grid in the above picture and calculate R[2][0] (the target cell is colored green).
// We should sum up the value of cells that can 't be reached by the cell (2, 0).
// These cells are colored yellow in this grid. So R[2][0] = 3 + 4 = 7.
//
...
// Input: grid = [[1]]
// Output: 0
// Explanation: Since there are no other cells than (0, 0), R[0][0] is equal to 0.
// So the sum of R[i][j] over all cells would be 0.
//
...
impl Solution {
    pub fn sum_remoteness(mut grid: Vec<Vec<i32>>) -> i64 {
        fn dfs(i: usize, j: usize, grid: &mut Vec<Vec<i32>>) -> (i64, i64) {
            let (mut s, mut t) = (grid[i][j] as i64, 1);
            grid[i][j] = 0;
            let (m, n) = (grid.len() as i32, grid[0].len() as i32);
            for d in [0, 1, 0, -1, 0].windows(2) {
                let (ni, nj) = (d[0] + i as i32, d[1] + j as i32);
                if ni < 0 || ni == m || nj < 0 || nj == n {
                    continue;
                }
                let (ni, nj) = (ni as usize, nj as usize);
                if grid[ni][nj] > 0 {
                    let (ns, nt) = dfs(ni, nj, grid);
                    s += ns;
                    t += nt;
                }
            }
        ...
        let cnt = grid
            .iter()
            .map(|row| row.iter().map(|&x| 0i64.max(x as i64)).sum::<i64>())
            .sum::<i64>();
