...
impl Solution {
    pub fn   minimum_semesters(n: i32, relations: Vec<Vec<i32>>) -> i32 {
        ...
        let mut q= std::collections::VecDeque::from((1..=n).filter(|x|!indegrees.contains_key(&x) ).collect::<Vec<i32>>());
        let mut courses=0;
        let mut semester=0;
        while !q.is_empty(){
...
mod test {
   ...
   pub fn  test_minimum_semesters_1() {
        assert_eq!(2, Solution::minimum_semesters(3, vec![vec![1,3],vec![2,3]]));
    }
 #[test]
   pub fn  test_minimum_semesters_2() {
        assert_eq!(-1, Solution::minimum_semesters(3, vec![vec![1,2],vec![2,3],vec![3,1]]));
    }
}
