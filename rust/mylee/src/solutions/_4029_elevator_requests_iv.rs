...
//  int elevator_requests(int n, int start, vector<vector<int>>& requests) {

struct Solution;
impl Solution {
    fn elevator_requests(n: i32, start: i32, requests: Vec<Vec<i32>>) -> i32 {
        0
    }
}
...
mod test {
    ...
    use crate::lc_matrix;
    #[test]
    pub fn test_elevator_requests_1() {
        assert_eq!(
            9,
            Solution::elevator_requests(9, 0, lc_matrix![[0, 8], [6, 5]])
        );
    }
    #[test]
    pub fn test_elevator_requests_2() {
        assert_eq!(
            7,
            Solution::elevator_requests(8, 5, lc_matrix![[1, 7], [7, 3]])
        );
    }
    #[test]
    pub fn test_elevator_requests_3() {
        assert_eq!(
            8,
            Solution::elevator_requests(7, 3, lc_matrix![[0, 5], [0, 1], [6, 3]])
        );
    }
}
