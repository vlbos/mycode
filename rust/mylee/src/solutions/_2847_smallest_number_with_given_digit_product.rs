...
// Input: n = 44
// Output: "-1"
// Explanation: It can be shown that there is no number such that its product of digits is equal to 44.
...
impl Solution {
    pub fn smallest_number(mut n: i64) -> String {
        let mut cnt = [0; 10];
        for i in (2..=9).rev() {
            while n % i == 0 {
                n /= i;
        ...
        let ans = (2..=9)
            .map(|i| i.to_string().repeat(cnt[i]))
            .collect::<Vec<_>>()
            .concat();
        if ans.is_empty() {
            String::from("1")
