...
impl Solution {
    ...
    pub fn find_closest_leaf2(root: Option<Rc<RefCell<TreeNode>>>, k: i32) -> i32 {
        ...
        let (graph, leaves) = tree_to_graph(root);
    ...
    fn find_closest_leaf3(root: Option<Rc<RefCell<TreeNode>>>, k: i32) -> i32 {
        ...
        fn preorder(
            root: &Option<Rc<RefCell<TreeNode>>>,
            parent: Option<i32>,
            nodes: &mut HashMap<i32, bool>,
            edges: &mut HashMap<i32, Vec<i32>>,
        ) {
            if let Some(r) = root {
                *nodes.entry(r.borrow().val).or_default() =
                ...
                preorder(&r.borrow().left, Some(r.borrow().val), nodes, edges);
                preorder(&r.borrow().right, Some(r.borrow().val), nodes, edges);
            }
        }
        preorder(&root, None, &mut nodes, &mut edges);
        vis.insert(k);
        queue.push_back(k);
