...
impl Solution {
        ...
        for j in 3..n - 3 {
            ...
            for k in j + 2..n - 1 {
                if prefix[n - 1] - prefix[k] == prefix[k - 1] - prefix[j]
                    && s.contains(&(prefix[k - 1] - prefix[j]))
                {
                    return true;
                }
            }
    ...
    pub fn split_array_error(nums: Vec<i32>) -> bool {
    ...
    pub fn split_arrayerror(nums: Vec<i32>) -> bool {
        let n = nums.len();
        if n < 7 {
            return false;
        }
...
mod test {
    use super::*;
    // [-6,8,-10,-10,-10,8,10,-2,6,6,0,2,-10,1,-4,-8,3,-2,7,-2]
    // false
    #[test]
