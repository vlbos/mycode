...
# | orange\_count | int  |
# +--------------+------+
# box\_id is the column with unique values for this table.
# chest\_id is a foreign key (reference column) of the chests table.
...
# | orange\_count | int  |
# +--------------+------+
# chest\_id is the column with unique values for this table.
...
def count_apples_and_oranges(boxes: pd.DataFrame, chests: pd.DataFrame) -> pd.DataFrame:
    ...
    apples = (df['apple_count_x'] + df['apple_count_y']).sum()
    oranges = (df['orange_count_x'] + df['orange_count_y']).sum()
    return pd.DataFrame({
        'apple_count': [apples],
