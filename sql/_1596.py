...
def most_frequently_products(customers: pd.DataFrame, orders: pd.DataFrame, products: pd.DataFrame) -> pd.DataFrame:
    return (
        orders.groupby(["customer_id", "product_id"], as_index=False)["order_id"]
        .count()
        .assign(
            rank=lambda df: df.groupby("customer_id")["order_id"].rank(
                ascending=False, method="dense"
            )
        )
        .query("rank == 1")
        .merge(products)[["customer_id", "product_id", "product_name"]]
    )