...
# Return _the result table_ _ordered by_ `points` _in **descending**, and then by_ `team_name` _in **ascending** order._
...
# +-------------------+--------+----------+---------+
# | team\_name         | points | position | tier    |
# +-------------------+--------+----------+---------+
...
def calculate_team_tiers(team_stats: pd.DataFrame) -> pd.DataFrame:
    ...
    team_stats = team_stats.sort_values(by=['points', 'team_name'], ascending=[False, True])
    
    total_teams = len(team_stats)

    team_stats['percentile'] = (team_stats['position'] - 1) / total_teams
    
    team_stats['tier'] = pd.cut(team_stats['percentile'], 
                                bins=[-float('inf'), 0.33, 0.66, float('inf')], 
                                labels=['Tier 1', 'Tier 2', 'Tier 3'])
    
    result = team_stats[['team_name', 'points', 'position', 'tier']]
