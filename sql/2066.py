...
# | amount      | int  |
# +-------------+------+
# (account\_id, day) is the primary key (combination of columns with unique values) for this table.
...
def account_balance(transactions: pd.DataFrame) -> pd.DataFrame:
    transactions['amount'] = np.where(transactions['type'] == 'Deposit', transactions['amount'], -transactions['amount']).astype(int)
    transactions = transactions.sort_values(by = ['account_id', 'day'], ascending = [1, 1])
