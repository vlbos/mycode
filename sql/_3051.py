...
def find_candidates(candidates: pd.DataFrame) -> pd.DataFrame:
    return candidates.query("skill=='PostgreSQL'")[['candidate_id']].merge(
    candidates.query("skill=='Python'")[['candidate_id']].merge(candidates.query("skill=='Tableau'")[['candidate_id']])).sort_values(by="candidate_id")