...
def acceptance_rate(friend_request: pd.DataFrame, request_accepted: pd.DataFrame) -> pd.DataFrame:
    reqNum = len(friend_request.drop_duplicates(["sender_id", "send_to_id"]))
    accNum = len(request_accepted.drop_duplicates(["requester_id", "accepter_id"]))
    return pd.DataFrame({"accept_rate": [accNum / reqNum if reqNum else 0]}).round(2)
