...
def shortest_distance(point2_d: pd.DataFrame) -> pd.DataFrame:
  df = point2_d.merge(point2_d, how = 'cross', suffixes =('_1','_2'))
  df = df.loc[~((df.x_1 == df.x_2) & (df.y_1 == df.y_2))]
  df['distance'] = ((df.x_1-df.x_2)**2 + (df.y_1-df.y_2)**2)**(1/2)
  return pd.DataFrame({'shortest': [df['distance'].min()]}).round(2)