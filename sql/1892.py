...
# **Output:** 
# +---------+---------+---------------+
# | user\_id | page\_id | friends\_likes |
# +---------+---------+---------------+
# | 1       | 77      | 2             |
# | 1       | 23      | 1             |
# | 1       | 24      | 1             |
...
def recommend_page(friendship: pd.DataFrame, likes: pd.DataFrame) -> pd.DataFrame:
    friend_table = pd.concat([friendship.rename(columns={'user1_id':'user_id','user2_id':'friend_id'}),friendship.iloc[:,[1,0]].rename(columns={'user2_id':'user_id','user1_id':'friend_id'})],ignore_index=True)
    result = friend_table.merge(likes,left_on='friend_id',right_on='user_id')[['user_id_x','friend_id','page_id']].rename(columns={'user_id_x':'user_id'})
    result = result.merge(likes,on=['user_id','page_id'],how='left',indicator=True)
    return result.query('_merge=="left_only"').groupby(['user_id','page_id']).size().to_frame(name='friends_likes').reset_index()