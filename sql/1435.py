...
# | duration            | int     |
# +---------------------+---------+
# session\_id is the column of unique values for this table.
...
# For session\_id 5 has a duration greater than or equal to 15 minutes.
...
def create_bar_chart(sessions: pd.DataFrame) -> pd.DataFrame:
    return pd.cut(
        ...
        bins=[0, 300, 600, 900, 2000]).value_counts().reset_index(name='total').rename(columns={'duration': 'bin'})