...
def find_requesting_users(signups: pd.DataFrame, confirmations: pd.DataFrame) -> pd.DataFrame:
    confirmations['time_24'] = confirmations.time_stamp + timedelta(hours=-24)
    df = confirmations.merge(confirmations, on='user_id', how='inner').query('time_24_y<=time_stamp_x<time_stamp_y')
