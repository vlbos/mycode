...
def number_of_joiners(candidates: pd.DataFrame) -> pd.DataFrame:
    candidates = candidates.sort_values(['experience','salary'], ascending=[False,True])
    candidates['temp'] = candidates.groupby(['experience'])['salary'].cumsum()
    candidates = candidates[candidates['temp'] <= 70000]
    candidates['temp2'] = candidates['salary'].cumsum()
    return candidates[candidates['temp2'] <= 70000][['employee_id']]
