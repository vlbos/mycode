...
def build_the_equation(terms: pd.DataFrame) -> pd.DataFrame:
    factor_string = terms.factor.astype(str)
    terms['factor'] = np.where(factor_string.str[0] != '-', '+' + factor_string, factor_string)
    power_string = terms.power.astype(str)
    terms['x'] = np.where(terms.power > 0, 'X', '') + np.where(terms.power > 1, '^' + power_string, '')
    terms.sort_values('power', ascending=False, inplace=True)
    return pd.DataFrame({
        'equation':[(terms['factor'] + terms['x']).sum() + '=0']
    })