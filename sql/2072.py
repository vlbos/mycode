...
# | score       | int  |
# +-------------+------+
# In SQL, student\_id is the primary key for this table.
...
def find_winner(new_york: pd.DataFrame, california: pd.DataFrame) -> pd.DataFrame:
    ny, ca = len(new_york.query("score>=90")), len(california.query("score>=90"))
    return pd.DataFrame({"winner": ["New York University" if ny>ca else "California University" if ny<ca else "No Winner"]})