...
def server_utilization_time(servers: pd.DataFrame) -> pd.DataFrame:
    servers = servers.sort_values(by=['status_time', 'session_status'])

    servers['stop_status_time'] = servers.groupby('server_id')['status_time'].shift(-1)
    servers = servers[servers['session_status'] == 'start'].rename(columns = {'status_time':'start_status_time'})
    servers['duration'] = (pd.to_datetime(servers['stop_status_time']) - pd.to_datetime(servers['start_status_time'])).dt.total_seconds()
    total_uptime_days = floor(servers['duration'].sum() / (24 * 60 * 60))
    return pd.DataFrame(data = [total_uptime_days], columns = ['total_uptime_days'])