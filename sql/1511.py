...
def customer_order_frequency(customers: pd.DataFrame, product: pd.DataFrame, orders: pd.DataFrame) -> pd.DataFrame:
    ...
    orders["order_date"]=pd.to_datetime(orders["order_date"],format=("%Y-%m-%d")).dt.strftime("%Y-%m")

    df=pd.merge(orders,product,on="product_id",how="left")
    df["amount"]=df["quantity"]*df["price"]
    df1=df.groupby(["customer_id","order_date"])["amount"].sum().to_frame("allamount").reset_index()
    df2=df1[df1["allamount"]>=100]
    df3=df2[df2["order_date"]=="2020-06"][["customer_id","order_date"]]
    df4=df2[df2["order_date"]=="2020-07"][["customer_id","order_date"]]
    df5=pd.merge(df3,df4,on="customer_id",how="inner")
    df6=pd.merge(df5,customers,on="customer_id",how="left")

    return df6[["customer_id","name"]]
