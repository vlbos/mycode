...
def hopper_company_queries(drivers: pd.DataFrame, rides: pd.DataFrame, accepted_rides: pd.DataFrame) -> pd.DataFrame:
    df = rides.assign(month = rides.requested_at.dt.month).merge(accepted_rides, on = "ride_id")
    df = df.query("requested_at.dt.year == 2020").merge(pd.DataFrame({"month":range(1,13)}),how="right",on="month")
    df = df[["month","ride_distance","ride_duration"]].fillna(0).groupby("month").sum().rolling(3).mean().round(2).reset_index()
    return df.assign(month = df.month-2).query("month > 0").rename(columns={col:"average_"+col for col in df.columns[1:]})