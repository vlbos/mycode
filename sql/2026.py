...
# | dislikes    | int  |
# +-------------+------+
# In SQL, problem\_id is the primary key column for this table.
...
def low_quality_problems(df: pd.DataFrame) -> pd.DataFrame:
    return df.sort_values(by='problem_id',ascending=True).loc[((df['likes']/(df['likes']+df['dislikes']))*100)<60,'problem_id'].reset_index()[['problem_id']]