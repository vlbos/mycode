...
def same_day_calls(calls: pd.DataFrame) -> pd.DataFrame:
    calls['date'] = calls['call_time'].dt.date
    c2 = calls[['recipient_id', 'caller_id', 'call_time', 'date']].rename(columns={'caller_id': 'recipient_id', 'recipient_id':'caller_id'})
    tmp = pd.concat([calls, c2], axis=0)
    summary = tmp.groupby(['caller_id', 'date'])[['call_time']].agg(['max', 'min']).reset_index().droplevel(1, axis=1)
    summary.columns = list(summary.columns[:2])+['max', 'min']
    tmp = summary.merge(tmp, on=['caller_id'])
    tmp[(tmp['max']==tmp['call_time'])|(tmp['min']==tmp['call_time'])].groupby('caller_id')['recipient_id'].count()
    return tmp[(tmp['max']==tmp['call_time'])|(tmp['min']==tmp['call_time'])].groupby(['caller_id', 'date_x'])['recipient_id'].nunique().reset_index().query('recipient_id==1')[['caller_id']].rename(columns={'caller_id': 'user_id'}).drop_duplicates()
