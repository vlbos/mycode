...
def find_maximum_transaction(transactions: pd.DataFrame) -> pd.DataFrame:
    df = transactions.assign(date = transactions.day.dt.to_period("D"))
    df["date_max_amount"] = df.groupby("date")["amount"].transform("max")
    return df.query("amount == date_max_amount").iloc[:,[0]].sort_values("transaction_id")
