...
# product\_id is a foreign key (column with unique values) to `Product` table.
...
def product_sales_analysis(sales: pd.DataFrame, product: pd.DataFrame) -> pd.DataFrame:
    ...
    return sales.groupby('user_id')['spending'].sum().reset_index().sort_values(by=['spending','user_id'],ascending=[False, True])