...
def calculate_team_dominance(teams: pd.DataFrame, passes: pd.DataFrame) -> pd.DataFrame:
    df=passes.merge(teams,how='left',left_on='pass_from',right_on='player_id').merge(teams,how='left',left_on='pass_to',right_on='player_id').rename(columns={'team_name_x':'team_name'})
    df['half_number']=(df.time_stamp>'45:00')+1
    df['dominance']=2*(df.team_name==df.team_name_y)-1
    return df.groupby(['team_name','half_number']).agg(dominance=('dominance','sum')).reset_index().sort_values(['team_name','half_number'])
