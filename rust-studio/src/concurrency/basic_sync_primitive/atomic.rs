...
pub fn relexed() {
    ...
    let _ = atomic_num.compare_exchange(old, 100, Ordering::SeqCst, Ordering::SeqCst);
...
//     let producer_thread = thread::spawn(move || {
//         // true
//         atomic_bool_clone.store(true, Ordering::AcqRel);
//     });
//  let atomic_bool_clone=atomic_bool.clone();
//     let consumer_thread = thread::spawn(move || {
//         // true
//         while !atomic_bool_clone.load(Ordering::AcqRel) {
//             // AcqRel
//         }
//         println!("Received value: true");
//     });
